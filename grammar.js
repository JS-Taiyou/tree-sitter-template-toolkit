module.exports = grammar({
  ...
  rules: {
    ...
    _value_expression: $ => choice(
      ...
      $.call_expression // <-- Added for function calls like .push()
    ),
    ...
    // --- NEW: A dedicated rule for function/method calls ---
    ...
    // --- MODIFIED: Added the string concatenation operator '_' ---
    ...
    // --- MODIFIED: A full implementation for hashes ---
    hash: $ => seq(
      '{',
      optional(seq(
        $.hash_pair,
    ...
    // --- NEW: A rule for a single key-value pair in a hash ---
    hash_pair: $ => seq(
